...
function displaynewdataset(finished=false) {
	if (finished == true) {
		...
		$('#submitnoidea').attr("disabled", true);
		$('.step1').hide();
		$('.step2').show();
	} else {
		if (annotation_data.length > 0) {
			...
			$("#titledisplay").html(current_item["title"]);
...
ajaxdata(true);
	...
	$("#titledisplay").click(function() {
		...
		if (t == ''){
			...
			current_item_span.push({"title_span": t.toString()});
			highlight(t.toString(),"titledisplay");
			$('#remove_spans').show();
		}
	});
